...
for i in 1 2; do
    ...
    for v in "21.11.18-1-601d05da" "22.1.5~rc1-1" "22.2.1~rc1-1"; do
        echo "$(date) installing $v" >> log
        ansible-playbook redpanda.install.yaml --extra-vars "redpanda_version=$v"
        ansible-playbook redpanda.configure.yaml
        ansible-playbook redpanda.start.yaml
        echo "$(date) testing suite-full-load-625k" >> log
        ansible-playbook test.yaml --extra-vars "suite=suite-full-load-625k"
