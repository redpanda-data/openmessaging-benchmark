...
def fault():
    ...
    try:
        ...
        if len(meta.topics) != 2:
            raise Exception(f"expected two topic, got: {meta.topics}")
        aggregatedEndToEndLatency95pct = [x for x in meta.topics if x != "__consumer_offsets"][0]
        ...
        for replica in info["replicas"]:
            if replica["node_id"] != info["leader_id"]:
                followers.append(replica["node_id"])
                if len(followers)>1:
                    new_configuration.append([replica["node_id"], replica["core"]])
            else:
                new_configuration.append([replica["node_id"], replica["core"]])
        for broker in meta.brokers:
            if broker.id == leader:
                continue
            if broker.id in followers:
                continue
            free.append(broker.id)
        print(f"[driver] topic: {topic}")
        print(f"[driver] leader: {leader}")
        print(f"[driver] follower: {followers}")
        print(f"[driver] free: {free}")
        new_configuration.append([free[0], 0])

        for broker in meta.brokers:
            if broker.id == leader:
                ip = broker.host
        payload = []
        for [id, core] in new_configuration:
            payload.append({
                "node_id": id,
                "core": core
            })
        print(f"[driver] reconfiguring to {json.dumps(payload)}")
        r = requests.post(f"http://{ip}:9644/v1/partitions/kafka/{topic}/0/replicas", json=payload)
        if r.status_code != 200:
            raise Exception(f"Can't reconfigure, status:{r.status_code} body:{r.text}")
        print("[driver] reconfiguring initiated")
    except:
        has_error = True
        e, v = sys.exc_info()[:2]
